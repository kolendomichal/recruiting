...
describe.each([2020, 2021, 2022])("calculatePrice.zero (%i)", (year: ServiceYears.ServiceYear) => {
...
])("calculatePrice.singleService (%s, %i)", (service: ServiceTypes.ServiceType, year: ServiceYears.ServiceYear, expectedPrice) => {
...
describe.each([
    ...
    [2021, 300],
    [2022, 0]
])("calculatePrice.photographyWithWeddingSessionPrice (%i increase by %i)", (year: ServiceYears.ServiceYear, increase) => {
    test("price matches requirements", () => {
...
describe.each([
    ...
    [2021, 300],
    [2022, 300]
])("calculatePrice.videoRecordingWithWeddingSessionPrice (%i increase by %i)", (year: ServiceYears.ServiceYear, increase) => {
    test("price matches requirements", () => {
...
describe.each([
    ...
    [2021, 500],
    [2022, 600]
])("calculatePrice.videoRecordingWithPhotographyPrice (%i increase by %i)", (year: ServiceYears.ServiceYear, increase) => {
    test("price matches requirements", () => {
...
describe.each([
    ...
    (year: ServiceYears.ServiceYear, increase) => {
        test("price matches requirements", () => {
...
describe.each([
  ...
  "calculatePrice.videoRecordingWithBlurayPackage (%i increase by %i)",
  (year: ServiceYears.ServiceYear, increase) => {
    test("price matches requirements", () => {
      const withoutBluray = calculatePrice(["VideoRecording"], year);
      
      const withBluray = calculatePrice(
        ["VideoRecording", "BlurayPackage"],
        year
      );

      const priceChangeWithBluray =
        withBluray.finalPrice - withoutBluray.finalPrice;

      expect(withBluray.basePrice).toBeGreaterThan(0);
      expect(withBluray.finalPrice).toBeGreaterThan(0);
      expect(priceChangeWithBluray).toEqual(increase);
    });

    test("bluray price reduced if no video recording selected", () => {
      const blurayWithPhotography = calculatePrice(["BlurayPackage", "Photography"], year);
      const onlyPhotography = calculatePrice(["Photography"], year);

      expect(blurayWithPhotography.finalPrice).toBe(onlyPhotography.finalPrice);
    });
  }
);
